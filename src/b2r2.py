...
def get_b2r2_l(
    ...
    for i in range(len(reactants_ncharges)):
        for j in range(len(reactants_ncharges[i])):
            arguments.append((i, j, reactants_ncharges[i][j], reactants_coords[i][j]))

    def func(args):
        i, j, ncharges, coords = args
        return i, j, get_b2r2_l_molecular(
            ncharges,
            coords,
            Rcut=Rcut,
            gridspace=gridspace,
    ...
    b2r2_l_reactants = [[] for _ in range(len(reactants_ncharges))]
    with ProcessPoolExecutor(max_workers=n_processes) as executor:
        results = list(tqdm(executor.map(func, arguments), total=len(arguments), desc="Reactants"))

    for i, j, result in results:
        b2r2_l_reactants[i].append(result)
    b2r2_l_reactants = np.sum(b2r2_l_reactants, axis=1)
